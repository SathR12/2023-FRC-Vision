...
class _ApriltagPose(ctypes.Structure):
    ...
    _fields_ = [("R", ctypes.POINTER(_Matd)), ("t", ctypes.POINTER(_Matd))]
...
class Detector:
    ...
    def __init__(
        ...
        if platform.system() == "Windows" and sys.version_info < (3, 8):
            ...
            WinDLL(str(pupil_pthreads_win.dll_path.resolve()))

        self.libc = None
        self.tag_detector = None
        self.tag_detector_ptr = None

        possible_hits = itertools.chain.from_iterable(
            Path(path).glob(filename_pattern) for path in searchpath
        )
        #print("POSSIBLE_HITS", possible_hits)
        for hit in possible_hits:
            ...
            if platform.system() == "Windows":
                ...
                if sys.version_info >= (3, 8):
                    #print(dll_path)
                    #RR commented dll_dir = os.add_dll_directory(dll_path)
                    #RR added below
                    dll_dir = os.environ['PATH'] + os.pathsep + dll_path
            #print("dll_dir")
...
if __name__ == "__main__":  # noqa
    ...
    at_detector = Detector(
        searchpath=["apriltags/lib", "apriltags/lib64"],
        families="tag36h11",
        nthreads=1,
        quad_decimate=1.0,
        quad_sigma=0.0,
    ...
    for image_name in image_names:
        ...
        cameraMatrix = numpy.array(parameters["multiple_tags_test"]["K"]).reshape(
            (3, 3)
        )
        camera_params = (
            cameraMatrix[0, 0],
